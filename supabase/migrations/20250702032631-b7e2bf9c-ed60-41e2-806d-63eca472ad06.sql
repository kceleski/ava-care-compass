-- Create missing tables for care planning tools (skipping existing care_types)
...
CREATE TABLE public.care_services (
  ...
  care_type_id UUID REFERENCES public.care_types(id),
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE TABLE public.emergency_contacts (
  ...
  name TEXT NOT NULL,
  phone TEXT NOT NULL,
  contact_type TEXT NOT NULL, -- 'medical', 'mental_health', 'general', 'local_service'
  description TEXT,
  available_24_7 BOOLEAN DEFAULT false,
  location_specific TEXT, -- state or city if location-specific
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
-- Care types - readable by all
CREATE POLICY "Care types are viewable by everyone" ON public.care_types FOR SELECT USING (true);

-- Location multipliers - readable by all
CREATE POLICY "Location multipliers are viewable by everyone" ON public.location_multipliers FOR SELECT USING (true);

-- Care services - readable by all
CREATE POLICY "Care services are viewable by everyone" ON public.care_services FOR SELECT USING (true);

-- User emergency contacts - user can manage their own
...
-- Update care_types table with additional fields if needed
ALTER TABLE public.care_types ADD COLUMN IF NOT EXISTS base_hourly_rate DECIMAL(8,2) DEFAULT 25.00;
ALTER TABLE public.care_types ADD COLUMN IF NOT EXISTS category TEXT;

-- Insert care types data
INSERT INTO public.care_types (name, description, base_hourly_rate, category) 
VALUES
